...
function renderNavPage() {
    for (let i = 0; i < pages.length; i++) {
        let liElement = $("<li>")
            ...
            .addClass("group relative w-full rounded border-2 border-[#583E26] px-1 py-0.5 text-center md:border-none md:p-0 nav-page ")
            .on("click", function () {
                setPage(pages[i]);
                setNavPage(pages[i]);
                setTitle(pages[i]);
        ...
        let divElement = $("<div>").addClass("absolute -bottom-1 h-1 bg-[#F7C815] duration-300 md:group-hover:w-full hidden md:block");

        if (pages[i] === pages[0]) {
            liElement.addClass("bg-[#583E26] md:bg-white text-white md:text-black");
...
function setNavPage(page) {
    if ($(window).innerWidth() <= 768) {
        $(".nav-page").each(function () {
            $(this).removeClass("bg-[#583E26] text-white").addClass("bg-white text-black");
            if ($(this).data("page") === page) {
                $(this).addClass("bg-[#583E26] text-white").removeClass("bg-white text-black");
            }
        });
    } else {
        $(".nav-page").each(function () {
...
function createProductElement(item) {
    let productElement = $("<div>").addClass("p-2 bg-white shadow-md shadow-gray-300 rounded-[14px] border-t-[2px] border-gray-100 flex flex-col justify-between");
    ...
    let titleElement = $("<p>").addClass("capitalize font-bold text-center text-base mt-2").text(item.name);

    let descriptionElement = $("<p>").addClass("text-xs").text(item.description);

    let sectionElement = $("<section>").addClass("flex items-center justify-between mt-3");
    ...
    let cartButtonElement = $("<p>").addClass("px-1 border-2 border-[#583e26] rounded-full text-xs hover:text-white md:hover:bg-[#583e26] duration-200 cursor-pointer btn-add-to-cart").text("add to cart").attr("data-id-product", item.id);
...
function renderProduct(product) {
    product.forEach((item) => {
        if (item.status_best_seller === true) {
            $("#wrapper-best-seller-product").append(createProductElement(item));
        }
    });
}
...
function getProductByCategory(category) {
    ...
    getJson("product", function (data) {
        ...
        for (let i = 0; i < product.length; i += groupsize) {
            ...
            groupNo += 1;
            let section = $("<section>").addClass("grid grid-cols-1 gap-2 md:gap-3 lg:gap-5 min-[470px]:grid-cols-2 min-[750px]:grid-cols-4 min-[1200px]:grid-cols-4 menu-group").attr("data-group-no", groupNo);
...
function setIndicatorGropMenu(current, maxGroupMenu) {
    ...
    if (maxGroupMenu > 1) {
        ...
        for (let i = 1; i <= maxGroupMenu; i++) {
            let button = $("<button>").addClass("h-3 w-3 rounded-full btn-indicator").attr("data-indicator-no", i);
...
function renderProductInCart(data) {
    let section = $("<section>").addClass("flex items-center justify-between gap-4 rounded-[6px] bg-white py-1 pl-1 pr-2 shadow mr-2");
    let img = $("<img>")
        .addClass("aspect-video w-20 rounded-sm")
        .attr("src", "../asset/" + data.img);
    let nameElement = $("<p>").addClass("w-24 overflow-hidden truncate text-ellipsis text-left text-xs md:text-sm font-semibold uppercase").text(data.name);
    let priceElement = $("<p>")
        .addClass("text-xs")
        .text(
    ...
    let buttonPlus = $("<button>")
        .addClass("flex h-4 w-4 items-center justify-center rounded-full bg-black text-xs text-white min-[1250px]:h-4 min-[1250px]:w-4 min-[1250px]:text-xs")
    ...
    let qty = $("<input>")
        .addClass(" h-6 w-6 rounded-full border-2 p-2 border-slate-200 text-xs min-[1250px]:h-6 min-[1250px]:w-9 min-[1250px]:text-xs input-product-cart")
        .val(data.quantity)
        .attr("data-id", data.id)
        .attr("type", "number")
        .off("click")
...
function setCart() {
    ...
    if (productInCart.length === 0) {
        ...
        let noItem = $("<section>").addClass("rounded-[6px] bg-white p-1 shadow mr-2 text-center font-semibold text-capitalize text-sm italic").text("Anda belum memasukkan menu apapun");
...
function setInfoDataUser() {
    ...
    $("#telepon-penerima").text(dataUSer.telepon !== "" ? dataUSer.kode + dataUSer.telepon : "-");
    ...
    if (checkProductisExist(product[0].id)) {
        cartProduct = cartProduct.map((item) => {
            if (item.id === product[0].id) {
                return { ...item, quantity: item.quantity + 1, totalPrice: item.quantity * item.price };
            }
            return item;
        });
    } else {
...
function deleteProductInCart(idProduct) {
    let cartProduct = getlocalStorage("product");
    let removeProduct = cartProduct.filter((product) => product.id !== idProduct);
    let selectedProduct = cartProduct.filter((product) => product.id === idProduct);
...
function checkDataUser() {
    ...
    if (userData.nama !== "" && userData.telepon !== "" && userData.alamat !== "" && userData.kode !== "+") {
        $("#submit-data-user")
            .removeClass("bg-gray-600")
            .addClass("bg-green-600")
            .off("click")
            .on("click", function () {
                setDataUser(userData.nama, userData.telepon, userData.alamat, userData.kode);
            });
        $("#btn-close-modal")
            .off("click")
            .on("click", function () {
        ...
        $("#submit-data-user").removeClass("bg-green-600").addClass("bg-gray-600").off("click");
        $("#btn-close-modal")
            .off("click")
            .on("click", function () {
                setDefaultDataUser();
...
function setDefaultDataUser() {
    postLocalStorage("dataUser", { nama: "", telepon: "", alamat: "", kode: "+62" });
    setInfoDataUser();
}
...
function checkOrder() {
    ...
    if (productInCart.length > 0 && dataUser.nama !== "" && dataUser.telepon !== "" && dataUser.alamat !== "" && dataUser.kode !== "+") {
        $("#btn-checkout")
        ...
        $("#btn-checkout").removeClass("bg-[#9c4a1a] md:hover:bg-[#583e26]").addClass("bg-gray-600 md:hover:bg-gray-700").off("click");
    }
}
